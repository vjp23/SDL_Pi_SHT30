...
class SHT30:
    ...
    def __init__(self, address=SHT30_I2CADDR, i2c=None, powerpin=0, fahrenheit=False, **kwargs):
    ...
    def _translate_temp(self, TRaw):
        if self.fahrenheit:
            # Fahrenheit, computed as 9/5 * (conversion to C) + 32, then simplified
            return ((TRaw * 315) / 65535.0) - 49
        # Celsius
        return ((TRaw * 175) / 65535.0) - 45

    # fast read for device detection without faults
    def _fast_read_data(self, crc=True):   
        ...
        HRaw = ((tmp[3] << 8) | tmp[4]) 
        self.temperature = self._translate_temp(TRaw)
        ...
        if crc:
            ...
            if (SHT30DEBUG == True):
                print("SHT30temperature=" + str(self.temperature))
                print("SHT30humdity=" + str(self.humidity))
                print("SHT30crcTR=" + str(self.crcT))
                print("SHT30crcTC=" + str(crcTC))
                print("SHT30crcHR=" + str(self.crcH))
                print("SHT30crcHC=" + str(crcHC))
    ...
    def _read_data(self):
        ...
        while count <= MAXREADATTEMPT:
            try:
                ...
                HRaw = ((tmp[3] << 8) | tmp[4]) 
                if (SHT30DEBUG == True):
                    print("TRaw = " + str(hex(TRaw)))
                    print("TRaw = " + str(TRaw))
                    print("tmp[0] = " + str(hex(tmp[0])))
                    print("tmp[1] = " + str(hex(tmp[1])))
                    print("tmp[0] = " + str((tmp[0])))
                    print("tmp[1] = " + str((tmp[1])))
                    print("tmp[0] & 0x7F" + str(hex(tmp[0] & 0x7F)))
                    print("TRaw*175/65535.0 = " + str(TRaw*175.0/65535.0 ))
                    print("TRaw*175/65535.0)-45 = " + str((TRaw*175.0/65535.0 )-45))
                ...
                if (self.SHT30PreviousTemp != -1000):   # ignore first time
                        if (self.humidity <0.01 or self.humidity > 100.0):
                            ...
                            if (SHT30DEBUG == True):
                                print(">>>>>>>>>>>>>")
                                print("Bad SHT30 Humidity = " + str(self.temperature))
                                print(">>>>>>>>>>>>>")
                ...
                if (SHT30DEBUG == True):
                    ...
                    print("SHT30readCount = " + str(count))
            count += 1
            self.retrys += 1
            time.sleep(0.10)
    ...
    def fast_read_humidity_temperature(self):
        self._fast_read_data(crc=False)
        return (self.humidity, self.temperature)
